...
public class Volvo : MonoBehaviour, ICarInteraction 
{
     ...
     private IEnumerator ToggleDoorCoroutine(Door door)
    {
        ...
        while (elapsedTime < duration)
        {
            ...
            door.transform.localEulerAngles = new Vector3(door.transform.localEulerAngles.x, angle, door.transform.localEulerAngles.z);
            yield return null;
        }

        door.transform.localEulerAngles = new Vector3(door.transform.localEulerAngles.x, targetRotation, door.transform.localEulerAngles.z);
